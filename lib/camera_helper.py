...
#sample_space=num_viewpoints, init_dist=1 init_elev=0
def init_viewpoints(mode, sample_space, init_dist, init_elev, principle_directions, 
    use_principle=True, use_shapenet=False, use_objaverse=False, use_face=False, pre_num=10):
...
def init_principle_viewpoints(
    ...
    if use_face:
        ...
        elev_list = [v for v in VIEWPOINTS[key]["elev"]]
        azim_list = [v for v in VIEWPOINTS[key]["azim"]]
        sector_list = [v for v in VIEWPOINTS[key]["sector"]]

        num_principle = pre_num
        dist_list = [dist_list[0] for _ in range(num_principle)]
        view_punishments = [0 for _ in range(num_principle)]
...
def init_camera(dist, elev, azim, image_size, device, offset=[0.0, 0.0, 0.0]):
    R, T = look_at_view_transform(dist, elev, azim)
    T = T + torch.tensor(offset).unsqueeze(0)
