...
def get_controlnet_depth(model_dir,training):
    print("=> initializing ControlNet Depth...")
    model, model_2, ddim_sampler = init_model(model_dir,training)
...
def apply_controlnet_depth(model, ddim_sampler, 
    ...
    device, blend=0, save_memory=False,resampling=0, first_image=None):
    """
    ...
    diffused_image_np = process(
        ...
        strength=strength, detected_map=depth_map_np, unknown_mask=np.array(generate_mask_image), save_memory=save_memory,resampling=resampling, first_image=first_image
    )[0]
    ...
    diffused_image_masked=diffused_image_np
