...
def build_diffusion_mask(mesh_stuff, 
    ...
    Texture_mask_tensor=(similarity_map_tensor >= view_threshold).float()
    ...
    Texture_mask_tensor=new_mask_tensor*Texture_mask_tensor
    ...
    # convert
    Texture_mask = Texture_mask_tensor[0].cpu().float().permute(2, 0, 1)
    Texture_mask = transforms.ToPILImage()(Texture_mask).convert("L")
    ...
    return Texture_mask,new_mask, update_mask, old_mask, exist_mask
...
def render_one_view(mesh,
    dist, elev, azim,
    image_size, faces_per_pixel,
    device, offset=[0.0,0.0,0.0]):

    # render the view
    cameras = init_camera(
        dist, elev, azim,
        image_size, device, offset
    )
...
def build_similarity_texture_cache_for_all_views(mesh, faces, verts_uvs,
    ...
    for i in tqdm(range(num_candidate_views)):
        cameras, _, _, _, similarity_tensor, _, _ = render_one_view(mesh,
            ...
            image_size, faces_per_pixel, device, offset)
...
def render_one_view_and_build_masks(dist, elev, azim, 
    ...
    image_size, faces_per_pixel,
    init_image_dir, mask_image_dir, normal_map_dir, depth_map_dir, similarity_map_dir,
    device, save_intermediate=False, smooth_mask=False, view_threshold=0.01, offset=[0.0,0.0,0.0]):
    ...
    Texture_mask_image, new_mask_image, update_mask_image, old_mask_image, exist_mask_image = build_diffusion_mask(
        (mesh, faces, verts_uvs), 
    ...
    return (
        ...
        old_mask_image, update_mask_image, new_mask_image, Texture_mask_image,
